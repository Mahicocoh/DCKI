export function parseMultipart(body, boundary) {
  ...
  const hasCrlf = body.indexOf(Buffer.from("\r\n")) !== -1;
  const nl = hasCrlf ? "\r\n" : "\n";
  const nlBuf = Buffer.from(nl);
  const headerSepBuf = Buffer.from(`${nl}${nl}`);
  ...
  while (pos !== -1) {
    ...
    if (body.slice(pos, pos + nlBuf.length).toString("utf8") === nl) pos += nlBuf.length;

    const headerEnd = body.indexOf(headerSepBuf, pos);
    if (headerEnd === -1) break;
    const headersText = body.slice(pos, headerEnd).toString("utf8");
    const dataStart = headerEnd + headerSepBuf.length;
    ...
    const dataEnd = body.slice(next - nlBuf.length, next).toString("utf8") === nl ? next - nlBuf.length : next;
    const data = body.slice(dataStart, dataEnd);

    const headers = {};
    for (const line of headersText.split(nl)) {
